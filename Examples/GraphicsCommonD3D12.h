...
void RegisterSrvHeap(const ComPtr<ID3D12Resource>& resource
...
void RegisterCBVHeap( const ComPtr<ID3D12Resource>& resource,
	const D3D12_CONSTANT_BUFFER_VIEW_DESC* cbvDesc, CD3DX12_CPU_DESCRIPTOR_HANDLE& cbvCPUHandle,
	CD3DX12_GPU_DESCRIPTOR_HANDLE& cbvGPUHandle);
void RegisterRtvHeap(const ComPtr<ID3D12Resource>& resource
