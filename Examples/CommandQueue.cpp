...
void GraphicsCommandQueue::Init(ComPtr<ID3D12Device> device, shared_ptr<SwapChain> swapChain)
{
	...
	device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, _cmdAlloc.Get(), nullptr, IID_PPV_ARGS(&m_cmdList));
	m_cmdList->Close();
...
void GraphicsCommandQueue::RenderBegin(ComPtr<ID3D12Resource>& renderTarget)
{
	m_cmdAlloc->Reset();
